...
func (db *appdbimpl) CommentPhoto(photoId string, c Comment_db) (Comment_db, error) {
	...
	_, err = db.c.Exec(`INSERT INTO comments (user_id, comment_id, created_in, body, photo_id, modified_in, is_reply_comment) VALUES (?,?,?,?,?,?)`, c.UserId, commentId, c.Created_in, c.Body, photoId, c.Modified_in, c.IsReplyComment)
	if err != nil {
